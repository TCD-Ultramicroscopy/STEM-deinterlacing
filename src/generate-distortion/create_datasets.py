...
for iii in range(0,21):
    ...
    in_image = tifffile.imread("generate-perfect/perfect.tif").astype(np.float64)
    ...
    for do_int in [False, True]:
        for do_rot in [False, True]:
            ...
            for dt, nf in zip(dwell_times, num_frames):
                ...
                if not do_int and not do_rot and (nf == 1 or nf == 40) and False:
                    plot_outputs = True
                else:
                    plot_outputs = False
